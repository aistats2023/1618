...
for i in range(1, num_iter):
    ...
    value_iter_error[i] = evaluate_value_func(ARCH_example, value_mat, value_cons)
...
obs, cost, next_obs = collect_samples(ARCH_example, 50000)
...
for i in range(2, num_iter):
    ...
    obs, cost, next_obs = collect_samples(ARCH_example, int(40000 * np.sqrt(i)))
    value_func.construct_next(obs, cost, next_obs, 
                                num_regressors = int(4 + i // 5), 
                                max_depth = 2, debias = False)
...
for i in range(1, num_iter):
    ...
    fvi_error[i] = evaluate_value_boost(ARCH_example, fvi_func)
...
# plt.title("ARCH policy evaluation, $\gamma = 0.99$", fontsize = 23)
...
# saves data
path = "../data/"
filename = "ARCH" + str(gamma) + "_numiter" + str(num_iter) + "_"
