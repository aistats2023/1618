...
# nonlinear system comes from a transformation of coordinates
...
def evaluate_value_boost(LQR_instance, value_function, num_samples = 10000):
    ...
    #
    # returns
    # mean squared error of the observed samples

    states, _, _ = collect_samples(LQR_instance, num_samples)
    true_values = np.zeros(num_samples)

    nonlin_states = convert_nonlinear(states)

    pred_values = value_function.evaluate(nonlin_states)
...
def convert_nonlinear(states):
    # converts the state in LQR coordinates to the nonlinear system
    # state: numpy array of size num_samples x 3 representing state in LQR coordinates
    #
    # returns
    # nonlinear_states: numpy array of size num_samples x 3 representing nonlinear coordinates

    state_one = states[:, 0] + np.power(states[:, 2], 2)
    state_two = states[:, 1] 
    state_three = states[:, 2] + np.power(states[:, 1] + np.power(states[:, 2], 2), 2)

    return np.vstack((state_one, state_two, state_three)).T
...
A = (np.random.rand(state_dim, state_dim) + np.random.rand(state_dim, state_dim)) / 10
B = np.random.rand(state_dim, action_dim) / 5
...
# runs value iteration on LQR coordinates
...
# run Krylov-Bellman on system in nonlinear coordinates

value_func = XG_LSTD_Boost(LQR_example.state_dim, discount = gamma, num_regressors = 300, max_depth = 7)
obs, cost, next_obs = collect_samples(LQR_example, 20000)

# converts to nonlinear coordinates
obs = convert_nonlinear(obs)
next_obs = convert_nonlinear(next_obs)

value_func.construct_first(obs, cost, num_regressors = 300, max_depth = 7)
value_func.construct_LSTD(obs, cost, next_obs)

# trains the Krylov-Bellman boosting algorithm
compute_oos_mse = False
debias = False
...
for i in range(2, num_iter):
    ...
    obs, cost, next_obs = collect_samples(LQR_example, int(20000))
    ...
    if i % 5 == 0 and compute_oos_mse:
        basis = value_func.lin_rep[-1]
        evaluate_oos(LQR_example, basis, num_check = 500)
...
fvi_func.first_fvi(obs, cost, num_regressors = 5, max_depth = 2)
...
# plt.title("Nonlinear policy evaluation, $\gamma = 0.9$", fontsize = 23)
# plt.savefig("plots/nonlin_gamma0.9.pdf", dpi=300)
# plt.show()
...
# saves data
path = "../data/"
filename = "nonlinear_lowsamples_" + str(gamma) + "_numiter" + str(num_iter) + "_"
