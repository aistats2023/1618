...
def plot(example, name, gamma):
    ...
    plt.savefig("plots/" + example + "_gamma" + str(gamma) + "_martin.pdf", dpi = 300)
    plt.close()
...
if krybell:
    plt.plot(np.log(data[1, :]), "bo--", label= "Krylov-Bellman Boosting")
    plt.plot(np.log(data[3, :]), "kx--", label = "Krylov-Bellman, low samples")
...
if krybell:
    filename = "fig_nonlinear_all_algs_09_martin.pdf"
else:
    filename = "fig_nonlinear_valit_only_09_martin.pdf"
